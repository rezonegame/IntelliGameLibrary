...
export class OpenAIProvider implements AiProvider {
  ...
  async init(apiKey: string, baseUrl?: string): Promise<boolean> {
    this.apiKey = apiKey;
  ...
  private async generateContent(prompt: string, isJson: boolean): Promise<any> {
    if (!this.apiKey) {
      throw new Error('OpenAI Provider has no API Key.');
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const message = errorData.error?.message || '调用 OpenAI 兼容 API 时发生未知错误';
      ...
      console.error('OpenAI Fetch Error:', error);
       if (!error.message.includes('API')) {
         this.toastService.show('网络请求失败，请检查您的网络连接或API端点', 'error');
       }
      throw error;
    }
  }
