              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="md:col-span-1">
                  ...
                  <div class="mt-4 grid grid-cols-2 gap-2">
                      ...
                      <button (click)="likeGame()" [disabled]="isLiked()" [title]="isLiked() ? '已点赞' : '点赞'"
                              class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-white text-slate-600 font-semibold rounded-lg shadow-md hover:bg-red-50 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-400 disabled:bg-red-50 disabled:text-red-500 border border-slate-200">
                ...
                <div class="md:col-span-2 space-y-6">
                  ...
                  <div class="mt-8">
                    <h3 class="text-xl font-bold text-cyan-700 mb-4">灵感工坊工具</h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <button (click)="requestInspiration('remodel')" class="text-left p-4 bg-slate-100 hover:bg-cyan-100/50 rounded-lg border border-slate-200 hover:border-cyan-300 transition-colors group">
                        ...
                        <p class="text-sm text-slate-500 mt-1">将《{{ game.name }}》代入一个新主题，看看会发生什么？</p>
                      </button>
                      <button (click)="requestInspiration('simulate')" class="text-left p-4 bg-slate-100 hover:bg-cyan-100/50 rounded-lg border border-slate-200 hover:border-cyan-300 transition-colors group">
                        ...
                        <p class="text-sm text-slate-500 mt-1">为《{{ game.name }}》引入一条新规则，让 AI 分析其影响。</p>
                      </button>
                    </div>
                  </div>
...
export class GameDetailModalComponent {
  ...
  requestInspiration(type: 'remodel' | 'simulate') {
    const game = this.game();
    if (game) {
      this.uiService.requestInspiration(game, type);
      this.close.emit();
    }
  }
}